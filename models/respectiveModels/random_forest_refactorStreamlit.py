...
class RandomForestModel:
    ...
    def plot_feature_importance(self):
        if not hasattr(self.model, 'feature_importances_') or self.feature_names is None:
            return None

        importances = self.model.feature_importances_
        feature_importance = pd.Series(importances, index=self.feature_names).sort_values()
    ...
    def plot_shap_summary(self, X_train, X_test):
        try:
            ...
            print(f"Could not generate SHAP plot for Random Forest: {e}")
            return None